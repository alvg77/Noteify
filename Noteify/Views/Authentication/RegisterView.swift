...
struct RegisterView: View {
    ...
    @ViewBuilder var registerButton: some View {
        let inactive = !registerVM.isValid || registerVM.registerProgress.inProgress
        ZStack {
            VStack {
                Button {
                    registerVM.register {
            ...
            .opacity(registerVM.registerProgress.inProgress ? 0.2 : 1)
            
            ProgressView()
                .opacity(registerVM.registerProgress.inProgress ? 1 : 0)
        }
    }
    
    @ViewBuilder var errorMessage: some View {
        HStack {
            if !registerVM.errorMessage.isEmpty { Image(systemName: "exclamationmark.triangle")
            }
